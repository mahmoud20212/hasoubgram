            ...
            <div class="border-b-2">
                <div class="flex items-center p-5 gap-3">
                    ...
                    @if ($post->owner->id === auth()->id())
                        ...
                        <form action="{{ route('posts.destroy', $post->id) }}" id="delete-post-form-{{ $post->id }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                    ...
                    @elseif (auth()->user()->isFollowing($post->owner))
                        <a href="{{ route('unfollow', $post->owner->username) }}" class="w-30 cursor-pointer text-sm font-bold py-1 px-3 text-center rounded text-red-500">
                            {{ __('Unfollow') }}
                        </a>
                    ...
                    @else
                        <a href="{{ route('follow', $post->owner->username) }}" class="w-30 cursor-pointer text-sm font-bold py-1 px-3 text-center rounded text-blue-500">
                            {{ __('Follow') }}
                        </a>
                    @endif
                </div>
            ...
            <div class="border-t p-5">
                <form action="/posts/{{ $post->slug }}/comment" method="POST">
                    ...
                    <div class="flex flex-row">
                        ...
                        class="h-10 grow resize-none overflow-hidden border-none bg-white text-blue-500 outline-0 focus:ring-0" required></textarea>
